...
async def list_ethereum_address_erc20_balances(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """
    List all ethereum addresses to monitor in the database and their respective ERC20 tokens and balances.
    """
    ...
    for address in addresses:
        ...
        for erc20_balance in erc20_balances:
            ...
            if erc20_balance.ethereum_token_balance is not None and erc20_balance.ethereum_token_decimals is not None:
                html_balance = "{:.2f}\n".format(float(erc20_balance.ethereum_token_balance) / pow(10, erc20_balance.ethereum_token_decimals))
            else:
                html_balance = f"0\n"
        ...
        logger.info(to_send)
        if len(to_send) > 4096:
            for x in range(0, len(to_send), 4096):
